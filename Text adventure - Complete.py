...
if leavebox.casefold() in ["check inv", "check inventory"]:
    ...
    if leavebox.lower() in ["y", "yes", "check inv", "check inventory"]:
        ...
        while True:
            gokart = input("Would you like to investigate the banana peel? Or try something else?\n (y/n): ").casefold()
            if gokart in ["y", "yes"]:
                StupidDeathEnd()
                break
...
bananaPeel = input("'HALT', someone yell's from behind you, do you want to turn around?\n (y/n):")
